...
```bash
python experiments/real_data_diagnostic.py --engression-model vanilla --target ccn --log-ccn --split paper --max-samples all --seq-stride 1 --train-size all --test-size all --epochs 80 --batch-size 256 --lr 0.01 --hidden-dim 128 --num-layer 5 --noise-dim 64 --prediction-samples 400 --device cuda --no-checkpoint --no-oos --out-dir runs/real_data_diagnostics/ena_weather/log10_ccn/paper/vanilla_best --skip-assertions
```
...
| Metric | Value |
|---|---:|
| energy score (CRPS, lower better) | `0.162226` |
| median abs error | `0.227323` |
| 90% interval coverage | `0.907453` |
| 50% interval coverage | `0.486957` |
| mean 90% interval width | `1.023936` |
| mean 50% interval width | `0.348820` |
