...
```bash
python experiments/real_data_diagnostic.py --target ccn --log-ccn --split paper --max-samples all --seq-stride 1 --train-size all --test-size all --epochs 120 --batch-size 256 --hidden-dim 192 --noise-dim 96 --num-layer 3 --stonet-head --lr 0.003 --weight-decay 0.0 --prediction-samples 800 --checkpoint-every 10 --device cuda --skip-assertions
```
