#!/usr/bin/env python
"""Aggregate the noise-in-the-encoder LSTM sweep into a leaderboard.
...
absolute error, and interval width alongside. The config (variant + lr + layers
+ hidden) is read from each run's folder name. Run after the array job finishes:
    ...
    .venv/bin/python slurm/real_data_lstm_stochastic_sweep/aggregate.py --sort-by energy
"""
...
DEFAULT_SWEEP_DIR = (
    ...
    / "log10_ccn" / "paper" / "lstm_stochastic_sweep"
)
