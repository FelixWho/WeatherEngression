# Real-Data Engression Run: ENA log10(CCN) / paper / vanilla
...
```bash
python experiments/real_data_diagnostic.py --engression-model vanilla --target ccn --log-ccn --split paper --max-samples all --seq-stride 1 --train-size all --test-size all --epochs 80 --batch-size 256 --lr 0.001 --hidden-dim 128 --num-layer 3 --noise-dim 128 --prediction-samples 400 --device cuda --out-dir runs/real_data_diagnostics/ena_weather/log10_ccn/paper/vanilla_sweep/lr0.001_h128_l3_n128 --skip-assertions
```
...
| Metric | Value |
|---|---:|
| energy score (CRPS, lower better) | `0.155618` |
| median abs error | `0.180313` |
| 90% interval coverage | `0.286957` |
| 50% interval coverage | `0.118012` |
| mean 90% interval width | `0.169820` |
| mean 50% interval width | `0.069963` |
...
| max_samples | `all` |
| seq_stride | `1` |
| sequence shape (train) | `(57646, 5302)` |
| train_size arg | `all` |
| test_size arg | `all` |
...
The `vanilla` engression model was fit; the training
tensor has shape `(57646, 5302)`. The `lstm` variant keeps
