...
```bash
python experiments/real_data_diagnostic.py --engression-model lstm --pre-additive --target ccn --log-ccn --split paper --max-samples all --seq-stride 1 --train-size all --test-size all --epochs 100 --early-stop-patience 12 --batch-size 256 --lr 0.003 --hidden-dim 128 --num-layer 3 --noise-dim 96 --prediction-samples 400 --oos-reference-size 2000 --oos-bins 10 --checkpoint-every 25 --device cuda --out-dir runs/real_data_diagnostics/ena_weather/log10_ccn/paper/lstm_sweep_oos/preadd_lr0.003_l3_h128 --skip-assertions
```
