...
```bash
python experiments/real_data_diagnostic.py --engression-model lstm --stochastic-init-noise --target ccn --log-ccn --split paper --max-samples all --seq-stride 1 --train-size all --test-size all --epochs 100 --early-stop-patience 12 --batch-size 256 --lr 0.003 --hidden-dim 192 --num-layer 5 --noise-dim 96 --prediction-samples 400 --device cuda --no-checkpoint --no-oos --out-dir runs/real_data_diagnostics/ena_weather/log10_ccn/paper/lstm_stochastic_sweep/init_state_lr0.003_l5_h192 --skip-assertions
```
