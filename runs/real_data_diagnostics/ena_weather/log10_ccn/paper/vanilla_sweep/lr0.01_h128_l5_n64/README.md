...
```bash
python experiments/real_data_diagnostic.py --engression-model vanilla --target ccn --log-ccn --split paper --max-samples all --seq-stride 1 --train-size all --test-size all --epochs 80 --batch-size 256 --lr 0.01 --hidden-dim 128 --num-layer 5 --noise-dim 64 --prediction-samples 400 --device cuda --out-dir runs/real_data_diagnostics/ena_weather/log10_ccn/paper/vanilla_sweep/lr0.01_h128_l5_n64 --skip-assertions
```
